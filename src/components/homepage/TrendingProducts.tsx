...
function ProductCard({ product }: { product: typeof products[0] }) {
  return (
    <div className="group rounded-lg border border-accent p-2">
      <div className="relative aspect-[3/4] overflow-hidden rounded-lg">
        ...
        {product.salePercentage && (
          <div className="absolute left-2 top-2 rounded-full bg-primary px-3 py-1 text-xs font-bold text-white">
            -{product.salePercentage}%
          </div>
        )}
        <button
          className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-foreground transition-colors hover:bg-white hover:text-primary"
      ...
      <Link href={`/product/${product.slug}`} className="mt-2 block text-left">
        <h3 className="font-heading text-base font-bold text-foreground text-left">
          {product.name}
        </h3>
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <button
          className="flex h-10 w-10 items-center justify-center rounded-full bg-foreground text-white transition-transform hover:scale-110 active:scale-95"
...
export function TrendingProducts() {
  ...
  return (
    <section className="w-full bg-gray-50 mt-0 lg:mt-24 pb-12 md:pb-24 lg:pb-32">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col lg:items-center">
          <div className="space-y-2">
            <h2 className="text-3xl font-bold tracking-tighter sm:text-5xl mb-2 lg:text-center">
              Trending Now
            </h2>
            <p className="hidden lg:block max-w-[900px] text-gray-500 md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed dark:text-gray-400 lg:text-center">
